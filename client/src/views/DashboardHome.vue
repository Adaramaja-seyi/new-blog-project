  ...
  methods: {
    async loadDashboardData() {
      ...
      try {
        ...
        await Promise.all([this.loadRecentPosts(), this.loadRecentComments()]);
      } catch (error) {
      ...
      try {
        ...
        if (response.data && response.data.stats) {
          this.stats = {
            ...
            totalViews: response.data.stats.views || 0,
            totalComments: response.data.stats.comments || 0,
            totalLikes: response.data.stats.likes || 0,
          };
        }
      } catch (error) {
    ...
    async loadRecentPosts() {
      // Get the latest 5 posts from the posts store
      await this.postsStore.fetchPosts({ limit: 5 });
      this.recentPosts = Array.isArray(this.posts)
    ...
    async deleteComment(commentId) {
      if (confirm("Are you sure you want to delete this comment?")) {
        try {
          this.recentComments = this.recentComments.filter(
